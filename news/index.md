...
- [`binom_sample_size()`](https://graemeleehickey.github.io/adaptDiag/reference/binom_sample_size.md):
  ...
  search. The previous formula used only $`p_1`$ in the variance term;
  the correct Chow et al. (2017) formula uses
  $`z_\alpha\sqrt{p_0(1-p_0)} + z_\beta\sqrt{p_1(1-p_1)}`$. The exact
  discrete search result is unchanged, but the approximation now matches
  the cited reference.
...
- [`multi_trial()`](https://graemeleehickey.github.io/adaptDiag/reference/multi_trial.md):
  added a `seed` argument for reproducible simulations
