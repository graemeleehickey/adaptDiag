...
* `multi_trial()` gains a `seed` argument for reproducible simulations, addressing a user request in #5. Reproducibility is implemented via `doRNG::%dorng%`, which correctly handles RNG stream splitting across all parallel backends.
