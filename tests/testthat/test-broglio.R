test_that("match broglio", {
  ...
  primary.analysis <- function(N, data, design.parameters, pr) {
    ...
    if (N.left > 0) {
      ...
      x.left.hist.pos.test.pos <- rbetabinom.ab(
        10000,
        x.left.hist.pos,
        pm$sens.alpha + tab2x2[2, 2],
        pm$sens.beta + tab2x2[2, 1]
      ...
      x.left.hist.neg.test.neg <- rbetabinom.ab(
        10000,
        x.left.hist.neg,
        pm$spec.alpha + tab2x2[1, 1],
        pm$spec.beta + tab2x2[1, 2]
  ...
  stop.check <- function(interim.analysis, design.parameters, pr) {
    go <- 1
    if (
      interim.analysis[2] >= design.parameters$min.hist.pos &
  ...
  sim.trial <- function(
    ...
    while (go == 1) {
      ...
      test.statistics <- primary.analysis(
        n.at.look,
        data,
        design.parameters,
        pr
  ...
  sim.N.trials.summarize <- function(
    ...
    t2 <- round(
      table(factor(
        results.matrix[, 2],
        levels = 1:3,
        labels = c("Futility", "MaxN", "EarlySuccess")
      )) /
        Nsims,
      3
    )
    t3 <- round(
      table(
        factor(results.matrix[, 3], design.parameters$looks),
        factor(results.matrix[, 1], labels = c("Fail", "Success"))
      ) /
        Nsims,
      3
    )
    ...
    t5 <- round(
      table(factor(
        as.numeric(results.matrix[, 6] >= design.parameters$sens.critv),
        levels = 0:1,
        labels = c("Lose Spec", "Win Spec")
      )) /
        Nsims,
      3
    )
  ...
  expect_equal(
    round(as.numeric(broglio[1]), 2),
    round(adaptDiag_out$power, 2),
    tolerance = 0.01
  )
  expect_equal(
    round(as.numeric(broglio[2]), 2),
    round(adaptDiag_out$stop_futility, 2),
    tolerance = 0.01
  )
  expect_equal(
    floor(as.numeric(broglio[5])),
    floor(adaptDiag_out$n_avg),
    tolerance = 5
  )
  expect_equal(
    round(as.numeric(broglio[6]), 2),
    round(adaptDiag_out$sens, 2),
    tolerance = 0.02
  )
  expect_equal(
    round(as.numeric(broglio[7]), 2),
