...
#' @param k integer. The \eqn{k}-th look, with \eqn{k = 1} denoting the first
#'   interim analysis.
...
analysis <- function(
  ...
  tab2x2 <- with(
    dat,
    table(
      factor(ref_pos, levels = c(0, 1)),
      factor(test_pos, levels = c(0, 1))
    )
  )
  dimnames(tab2x2) <- list(ref_pos = c("0", "1"), test_pos = c("0", "1"))
  ...
  if (k < n_stages) {
    ...
    ref_pos_test_pos_remain <- rbbinom(
      n_mc,
      size = ref_pos_remain,
      alpha = alpha_sens,
      beta = beta_sens
    ...
    ref_neg_test_neg_remain <- rbbinom(
      n_mc,
      size = ref_neg_remain,
      alpha = alpha_spec,
      beta = beta_spec
    ...
    ppp_succ_both <- mean(
      (pp_sens_max_n >= succ_sens) & (pp_spec_max_n >= succ_spec)
    )
  } else {
